...
function build_kernel() {
    ...
    local afterlife_version="afterlife-$(_get_build_var_cached AFTERLIFE_VERSION)"
    ...
    if [[ "${SKIP_KERNEL_SYNC}" != "true" && "${SKIP_KERNEL_SYNC}" != "1" ]]; then
        ...
        local repo_init_args=("-b" "${afterlife_version}")
        if [ -n "${AFTERLIFE_MIRROR}" ]; then
            repo_init_args+=("--reference" "${AFTERLIFE_MIRROR}")
        fi
        if [ -n "${REPO_VERSION}" ]; then
            repo_init_args+=("--repo-rev" "${REPO_VERSION}")
        fi

        yes | repo init -u https://github.com/AfterlifeOS/${target_kernel_manifest}.git ${repo_init_args[@]} || [ $? -eq 141 ]
        if [ $? -ne 0 ]; then
